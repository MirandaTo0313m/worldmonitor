...
// in the main JS chunk. `en.rest.json` covers per-panel content and is
// dynamically imported during init() — Vite emits it as a separate chunk
// so it's parsed off the critical path. Both are awaited before init
// resolves, so every t() call sees the full merged dictionary by the
// time any panel renders. Source of truth stays at en.json; the two
// derived files are regenerated by scripts/build-i18n-shell.mjs (wired
// into prebuild) and a parity test blocks drift.
